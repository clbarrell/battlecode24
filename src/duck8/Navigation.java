...
public class Navigation extends RobotPlayer {
    ...
    static class BugNav {
        ...
        static boolean move() throws GameActionException {
            try {
                ...
                if (d < minDistToEnemy) {
                    m += ("New min dist: " + d + " Old: " + minDistToEnemy + ". ");
                    if (lastObstacleFound != null) {
                        m += "broke free from!!: " + lastObstacleFound + ". ";
                        resetPathfinding();
                    }
                    minDistToEnemy = d;
                }
                ...
                if (lastObstacleFound != null) {
                    m += "Last obstacle found: " + lastObstacleFound + ". ";
                ...
                if (canMove(dir)) {
                    ...
                    if (rc.onTheMap(newLoc)) {
                        ...
                        if (!mi.isWall() && !mi.isWater() && !isRobot && roundNum <= GameConstants.SETUP_ROUNDS) {
                            m += "Cant go " + pl(newLoc) + " probs barrier. ";
                        ...
                        if (mi.isWater()) {
                            ...
                            if (cellsWithWater < maxWaterCells || iAmWaterBreaker) {
                                ...
                                if (mp.x % 2 == mp.y % 2) {
                                    ...
                                    MapInfo mi2 = turnRight ? rc.senseMapInfo(myLocation.add(dir.rotateRight()))
                                            : rc.senseMapInfo(myLocation.add(dir.rotateLeft()));
                                    mp = mi2.getMapLocation();
                                }

                                if (rc.canMove(myLocation.directionTo(mp))) {
                                    m += "Moving around the water. ";
                ...
                if (lastObstacleFound == null && (myState != States.ATTACKING || myState != States.MOVING_TO_ATTACK)) {
                    m += "Checking best dir for wall. ";
                    int dR = INF;
                    int dL = INF;
                    ...
                    m += "Looking " + (rotateRight ? "right" : "left") + " till I can move. ";
                    for (int j = 8; j-- > 0;) {
                        if (rotateRight) {
                            dir = dir.rotateRight();
                        } else {
                        ...
                        if (canMove(dir)) {
                            if (lastLoc == null) {
                                lastLoc = myLocation;
                                m += "Can move, not my old location. ";
        ...
        private static Direction chooseBestDir() {
            ...
            int lDist = lLast.distanceSquaredTo(target);
            int rDist = rLast.distanceSquaredTo(target);
            m += "L dist: " + lDist + ", R dist: " + rDist + ". ";
            if (lDist < rDist) {
                rotateRight = false;
                lastObstacleFound = lastLObstacle;
                m += "[L] closer. Setting lastObsFound to " + lastObstacleFound + ". ";
                return myLocation.directionTo(lPath.get(0));
            } else if (rDist < lDist) {
                rotateRight = true;
                lastObstacleFound = lastRObstacle;
                m += "[R] closer. Setting lastObsFound to " + lastObstacleFound + ". ";
                ...
                if (lPath.size() < rPath.size()) {
                    rotateRight = false;
                    lastObstacleFound = lastLObstacle;
                    m += "[L] shorter path. Setting lastObsFound to " + lastObstacleFound + ". ";
                    return myLocation.directionTo(lPath.get(0));
                } else {
                    rotateRight = true;
                    lastObstacleFound = lastRObstacle;
                    m += "[R] shorter path. Setting lastObsFound to " + lastObstacleFound + ". ";
                    return myLocation.directionTo(rPath.get(0));
                }
            }
        }
        ...
        static MapLocation getNextPassableDirection(boolean right, Direction dir) throws GameActionException {
            ...
            // Debug.log(
            // m += "Checking " + (right ? "right" : "left") + " dir: " + dir.toString() + "
            // from" + pl(start) + ". ";
            for (int j = 8; j-- > 0;) {
                if (right) {
                    dir = dir.rotateRight();
                ...
                if (canMove) {
                    // m += " Next dir for " + (right ? "right" : "left") + " is " + dir + " to" +
                    // pl(newLoc) + ".\n";
                    return newLoc;
                } else {
                    rc.setIndicatorDot(newLoc, 255, 180, 180);
    ...
    public static MapLocation bestEnemyLocationGuess() throws GameActionException {
        // check if I need to support flag capture
        MapLocation friendToSupport = Comm.flagCaptureSupportNeeded();
        if (friendToSupport != null) {
            // Debug.log("Going to support flag capture: " + friendToSupport.toString());
            botLog += "Going to support flag carrier. ";
            setState(States.SUPPORTING_FLAG_CARRIER);
            return friendToSupport;
        }
